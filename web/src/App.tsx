...
function PageContent({ page, t, token, traders, credentials, templates, onSelectTrader, onChanged }: { page: Page; t: Copy; token: string; traders: Trader[]; credentials: Credential[]; templates: Template[]; onSelectTrader: (id: string) => void; onChanged: () => void }) {
  ...
  if (page === "admin") return <AdminPage t={t} token={token} traders={traders} credentials={credentials} onSelect={onSelectTrader} onChanged={onChanged} />
  return <Overview t={t} token={token} traders={traders} credentials={credentials} onTraders={() => onSelectTrader("")} onChanged={onChanged} />
}

function Overview({ t, token, traders, credentials, onTraders, onChanged }: { t: Copy; token: string; traders: Trader[]; credentials: Credential[]; onTraders: () => void; onChanged: () => void }) {
  const failed = traders.filter(trader => trader.status === "failed").length
  return <div className="flex flex-col gap-6"><div><h1 className="m-0 text-2xl font-semibold tracking-tight">{t.overview}</h1><p className="mt-1 text-sm text-muted-foreground">{t.usage}</p></div><div className="grid gap-4 sm:grid-cols-3"><Metric label={t.traders} value={traders.length} /><Metric label={t.credentials} value={credentials.length} /><Metric label={t.failed} value={failed} tone={failed ? "danger" : "default"} /></div><Card><CardHeader><CardTitle>{t.traders}</CardTitle><CardDescription>{t.updated}</CardDescription></CardHeader><CardContent>{traders.length ? <TraderTable t={t} token={token} traders={traders.slice(0, 5)} onSelect={() => onTraders()} onChanged={onChanged} /> : <EmptyState title={t.noTraders} description={t.usage} icon={WorkflowIcon} />}</CardContent></Card></div>
}
...
function TraderEnabledSwitch({ trader, token, t, onChanged }: { trader: Trader; token: string; t: Copy; onChanged: () => void }) {
...
function AdminPage({ t, token, traders, credentials, onSelect, onChanged }: { t: Copy; token: string; traders: Trader[]; credentials: Credential[]; onSelect: (id: string) => void; onChanged: () => void }) { const owners = new Set([...traders, ...credentials].map(item => item.owner_id)); return <div className="flex flex-col gap-6"><div><h1 className="m-0 text-2xl font-semibold tracking-tight">{t.admin}</h1><p className="mt-1 text-sm text-muted-foreground">Root can inspect all durable resources without receiving credential secrets or signal tokens.</p></div><div className="grid gap-4 sm:grid-cols-3"><Metric label={localeText(t, "Users", "用户")} value={owners.size} /><Metric label={t.traders} value={traders.length} /><Metric label={t.credentials} value={credentials.length} /></div><Card><CardHeader><CardTitle>{t.traders}</CardTitle><CardDescription>{localeText(t, "All users", "全部用户")}</CardDescription></CardHeader><CardContent><TraderTable t={t} token={token} traders={traders} onSelect={onSelect} onChanged={onChanged} /></CardContent></Card></div> }
...
function TraderDetail({ trader, token, t, onBack, onChanged }: { trader: Trader; token: string; t: Copy; onBack: () => void; onChanged: () => void }) {
